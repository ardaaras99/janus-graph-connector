...
class JanusGraphClient:
    ...
    def _connection(self) -> Generator[GraphTraversalSource, None, None]:
        ...
        try:
            connection = DriverRemoteConnection(
                self.gremlin_url, "g", message_serializer=self.serializer
            )
            ...
            with self._connection() as g:
                for node_name, data_dict in schemas_dict.items():
                    ...
                    if existing_vertex:
                        ...
                        vertex.iterate()
                        print(
                            f"Updated existing node for '{node_name}' in ontology '{ontology_name}' version '{version}'"
                        )
                    else:
                        # Create new node
                        ...
                        vertex.iterate()
                        print(
                            f"Created new node for '{node_name}' in ontology '{ontology_name}' version '{version}'"
                        )

                print(
                    f"Successfully saved {len(schemas_dict)} schema nodes to JanusGraph for ontology '{ontology_name}' version '{version}'"
                )
        ...
        try:
            ...
            with self._connection() as g:
                for edge_label, value in schemas_dict.items():
                    ...
                    # Create edge
                    g.V().hasLabel(source_node_title).has("ontology", ontology_name).has(
                        "version", version
                    ).as_("s").V().hasLabel(target_node_title).has("ontology", ontology_name).has(
                        "version", version
                    ).as_("t").addE(edge_label).from_("s").to("t").property(
                        "schema", schema_json
                    ).property("ontology", ontology_name).property("version", version).iterate()

                    print(
                        f"Created new edge between '{source_node_title}' and '{target_node_title}' with label '{edge_label}' for ontology '{ontology_name}' version '{version}'"
                    )
    ...
    def read_nodes(
        self, ontology_name: str, version: str
    ) -> dict[str, tuple[BaseNode, bool, str]] | None:
        """
        ...
        try:
            with self._connection() as g:
                ...
                print(
                    f"Successfully read {len(node_dict)} nodes from ontology '{ontology_name}' version '{version}'"
                )
                return node_dict
    ...
    def read_relations(self, ontology_name: str, version: str) -> dict[str, BaseRelation] | None:
        ...
        try:
            with self._connection() as g:
                ...
                print(
                    f"Successfully read {len(relation_dict)} relations from ontology '{ontology_name}' version '{version}'"
                )
        ...
        try:
            with self._connection() as g:
                if ontology_name and version:
                    ...
                    print(
                        f"Removed all vertices for ontology '{ontology_name}' version '{version}'"
                    )
